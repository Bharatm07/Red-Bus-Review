...
def get_unique_routes():
    conn = None
    try:
        conn = psycopg2.connect(host='dpg-d1n7mr7diees73esisqg-a.oregon-postgres.render.com',database='red_bus_database',user='red_bus_database_user',password='zO8CB4x0ZEAN7BAbeQ1GujEwhZb5qjVG',port=5432)
        conn.autocommit = True  # For read-only queries
        cursor = conn.cursor()
        
        cursor.execute('SELECT DISTINCT route_name FROM "ap" ORDER BY route_name')
...
<div style="text-align: center; color: #666; padding: 2rem;">
    <p>Developed with ❤ by <strong>Bharat Muthukumar</strong></p>
</div>
""", unsafe_allow_html=True)